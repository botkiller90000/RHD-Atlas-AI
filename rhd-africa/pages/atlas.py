...
def render():
    ...
    with tab_atlas:
        selected_q = st.selectbox("Jump to Question", QUESTIONS, index=0)
...
def _q4(gbd, who, wb):
    ...
    colors_age = [DARK["blue"] if "15-29" in a or "5-14" in a else DARK["muted"] for a in agg["age_group"].astype(str)]
    fig = go.Figure(go.Bar(
...
def _q9(gbd, who, wb):
    ...
    merged = who_mort.merge(wb_agg, on="country")
    merged = merged.merge(who[["country", "region"]].drop_duplicates(), on="country")
    fig = go.Figure()
    for reg, color in REGION_COLORS.items():
...
def _q10(gbd, who, wb):
    ...
    merged = who_prev.merge(wb_agg, on="country")
    merged = merged.merge(who[["country", "region"]].drop_duplicates(), on="country")
    fig = go.Figure()
    for reg, color in REGION_COLORS.items():
        sub = merged[merged["region"] == reg]
...
def _q11(gbd, who, wb):
    ...
    top = top.merge(who[["country", "region"]].drop_duplicates(), on="country")
...
def _q19(gbd, who, wb):
    ...
    merged = who_agg.merge(wb_agg, on="country")
    merged = merged.merge(who[["country", "region"]].drop_duplicates(), on="country")
    corrs = {
        "Health Exp/Capita": merged["prevalence_per_100k"].corr(merged["health_expenditure_per_capita"]),
        "Physician Density": merged["prevalence_per_100k"].corr(merged["physician_density_per_1000"]),
        "Urbanisation": merged["prevalence_per_100k"].corr(merged["urban_population_pct"]),
        "Hospital Beds": merged["prevalence_per_100k"].corr(merged["hospital_beds_per_1000"]),
    }
