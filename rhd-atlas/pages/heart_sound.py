...
def render():
    ...
    with st.expander("ℹ️ How this works"):
        st.markdown("""
        **Pipeline overview:**
        1. Audio upload → preprocessing → noise reduction → bandpass filtering → resampling
        2. Segmentation → feature extraction (Mel Spectrogram, MFCC, Spectral Centroid, Zero Crossing Rate, Chroma, RMS Energy)
        3. Machine learning classifier → prediction → confidence score → clinical recommendation

        **Output classes:**
        - **Normal** — Regular S1/S2 cardiac cycle, no significant murmur
        - **Innocent Murmur** — Functional murmur, no structural pathology
        - **Pathological Murmur** — Structural cardiac abnormality detected
        - **RHD Likely** — Acoustic profile consistent with rheumatic valvular disease

        > ⚠️ This is a demonstration system using simulated acoustic feature extraction.
        > A production system would use trained CNN/LSTM models on labelled PCG datasets (e.g., PhysioNet Challenge).
        """)
    ...
    if st.session_state.hs_results is not None:
        ...
        guidance_map = {
            "Normal": (DARK["green"], "Routine follow-up. No immediate action required. Patient education on GAS pharyngitis recognition advised."),
            "Innocent Murmur": (DARK["yellow"], "Repeat auscultation in 6–12 months. No echocardiogram required unless symptoms develop."),
            "Pathological Murmur": (DARK["accent"], "Echocardiographic evaluation within 4 weeks. Cardiology referral if pathology confirmed. Consider secondary prophylaxis."),
            "RHD Likely": ("#ff6e6e", "Urgent cardiology referral. Echocardiogram within 1 week. Initiate benzathine penicillin G prophylaxis pending confirmation."),
        }
        color, text = guidance_map[predicted]
        st.markdown(
    ...
    st.caption(
        "⚠️ This tool is for educational and research purposes only and does not constitute "
        "medical advice. All classifications must be interpreted by qualified clinicians."
    )
